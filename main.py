...
class Bill_App:
    ...
    #GroceryItems
        ...
        biscuit_lbl = Label(F3, text="biscuit", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
        biscuit_lbl.grid(row=2, column=0, padx=10, pady=10, sticky='W')
        biscuit_txt = Entry(F3, width=10, textvariable=self.biscuit, font=('times new roman', 16, 'bold'), bd=5, relief=GROOVE)
        biscuit_txt.grid(row=2, column=1, padx=10, pady=10)
    ...
    #ColdDrinks
        ...
        biscuit_txt = Entry(F4, width=10, textvariable=self.mazza, font=('times new roman', 16, 'bold'), bd=5, relief=GROOVE)
        biscuit_txt.grid(row=2, column=1, padx=10, pady=10)
...
#totalBill
    def total(self):
        ...
        self.g_w_p = self.biscuit.get()*10
...
#billArea
    ...
    #Grocery
        ...
        if self.biscuit.get() != 0:
            self.txtarea.insert(END, f"\n biscuit\t\t{self.biscuit.get()}\t\t{self.g_w_p}")
        if self.daal.get() != 0:
    ...
    def find_bill(self):
        present = "no"
        for i in os.listdir("bills/"): #os.listdir(path): Returns a list of all files and directories in the specified directory.
